	...
	BX.Sale.OrderAjaxComponent = {
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-desc-container'}}),
				...
				clear, infoList, extraServices, extraServicesNode, warehouses;
			...
			if(currentDelivery.ID == 5 || currentDelivery.ID == 6){
				...
				console.log("editDeliveryInfo ORDER_PROP_7");
				$("body").find("textarea[name=ORDER_PROP_7]").val("");
			}*/
			...
			deliveryInfoContainer.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-pp-company'},
					children: [subTitle, label, title, clear, extraServicesNode, infoList, warehouses]
				})
			);
		...
		getWarehouses: function(delivery_id){
			...
			if($("#bx-soa-order").data("warehouse") != null || $("#bx-soa-order").data("warehouse") != undefined ) {
				warehouses = $("#bx-soa-order").data("warehouse");
			}

			BX.ajax({
				method: 'POST',
				dataType: 'html',
				url: "/local/ajax/np_warehouses_data.php", //this.ajaxUrl
				data: {
				...
				onsuccess: BX.delegate(function(result) {
					$("body").find(".bx-soa-pp-warehouses").empty().append(result);
					var warehouse = $("body").find("#warehouse-list").find("option:selected").data("wh-ref");
					$("#bx-soa-order").data("warehouse", warehouse);
					//console.log("getWarehouses ORDER_PROP_7");
					//$("body").find("textarea[name=ORDER_PROP_7]").val($("body").find("#warehouse-list").val());
...
$(document).ready(function(){
	$("body")
		.on("change", "#warehouse-list", setWarehouse)
	;
});
...
function setWarehouse(){
	...
	var warehouse = $("body").find("#warehouse-list").find("option:selected").data("wh-ref");
	$("#bx-soa-order").data("warehouse", warehouse);
	//console.log("setWarehouse ORDER_PROP_7");
	//$("body").find("textarea[name=ORDER_PROP_7]").val($("body").find("#warehouse-list").val());
}